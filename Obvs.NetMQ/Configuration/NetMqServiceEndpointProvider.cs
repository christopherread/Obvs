...
namespace Obvs.NetMQ.Configuration
{
    public class NetMqServiceEndpointProvider<TServiceMessage> : ServiceEndpointProviderBase where TServiceMessage : IMessage
    {
        ...
        public NetMqServiceEndpointProvider(string serviceName, string address, int port, IMessageSerializer serializer, IMessageDeserializerFactory deserializerFactory)
            : base(serviceName)
        {
        ...
        public override IServiceEndpoint CreateEndpoint()
        {
            return new ServiceEndpoint(
               new MessageSource<IRequest>(_requestAddress, _deserializerFactory.Create<IRequest, TServiceMessage>(), _context, RequestsDestination),
               new MessageSource<ICommand>(_commandAddress, _deserializerFactory.Create<ICommand, TServiceMessage>(), _context, CommandsDestination),
        ...
        public override IServiceEndpointClient CreateEndpointClient()
        {
            return new ServiceEndpointClient(
               new MessageSource<IEvent>(_eventAddress, _deserializerFactory.Create<IEvent, TServiceMessage>(), _context, EventsDestination),
               new MessageSource<IResponse>(_responseAddress, _deserializerFactory.Create<IResponse, TServiceMessage>(), _context, ResponsesDestination),
